...
const BrowseArts: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Exclude arts by the logged-in artist
      const filteredArts = artsResponse.data.filter(
        (art) =>
          art.artist && art.artist.firstname && user.id !== art.artist.user_id
      );
  ...
  const filteredArts = arts.filter((art) => {
    ...
    const matchesPriceRange =
      ...
      (() => {
        ...
        if (selectedPriceRange === "10000-20000")
    ...
    const matchesStyle =
      ...
      selectedStyles.includes(art.art_style || "");
    const matchesMedium =
      selectedMediums.length === 0 ||
      selectedMediums.includes(art.medium || "");
    const matchesSubject =
      selectedSubjects.length === 0 ||
      selectedSubjects.includes(art.subject || "");
    const matchesTags =
      selectedTags.length === 0 ||
      art.tags.some((tag) => selectedTags.includes(tag.name));
  ...
  const handleFilterToggle = (
    filter: "price" | "style" | "medium" | "subject" | "tags"
  ) => {
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6">
          ...
          <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4">
            <button
              ...
              onClick={() =>
                setSortOption(sortOption === "latest" ? "" : "latest")
              }
            >
              Latest
            </button>
            <button
              ...
              onClick={() =>
                setSortOption(sortOption === "cheapest" ? "" : "cheapest")
              }
            >
              Cheapest
            </button>
          ...
          {filterVisible && (
            <aside className="w-full md:w-64 p-4 bg-white border-r shadow-md rounded-lg transition-all duration-300 mb-4 md:mb-0">
              ...
              <div className="mb-6">
                ...
                >
                  Price Range{" "}
                  {openFilters.price ? <FaChevronUp /> : <FaChevronDown />}
                </h3>
                {openFilters.price && (
                  <div className="mt-2">
              ...
              <div className="mb-6">
                ...
                >
                  Art Style{" "}
                  {openFilters.style ? <FaChevronUp /> : <FaChevronDown />}
                </h3>
                {openFilters.style && (
                  <div className="mt-2">
                    {artStyles.map((style) => (
                      <label key={style} className="block mb-2">
                        <input
                          ...
                          onChange={() =>
                            handleCheckboxChange(setSelectedStyles, style)
                          }
                          className="mr-2"
                        />
                        {style}
              ...
              <div className="mb-6">
                ...
                >
                  Medium{" "}
                  {openFilters.medium ? <FaChevronUp /> : <FaChevronDown />}
                </h3>
                {openFilters.medium && (
                  <div className="mt-2">
                    {mediums.map((medium) => (
                      <label key={medium} className="block mb-2">
                        <input
                          ...
                          onChange={() =>
                            handleCheckboxChange(setSelectedMediums, medium)
                          }
                          className="mr-2"
                        />
                        {medium}
              ...
              <div className="mb-6">
                ...
                >
                  Subject{" "}
                  {openFilters.subject ? <FaChevronUp /> : <FaChevronDown />}
                </h3>
                {openFilters.subject && (
                  <div className="mt-2">
                    ...
                    {tags.map((tag) => (
                      <label key={tag.id} className="flex items-center mb-2">
                        <input
                          ...
                          onChange={() =>
                            handleCheckboxChange(setSelectedTags, tag.name)
                          }
                          className="mr-2"
                        />
                        {tag.name}
          ...
          >
            {paginatedArts.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-gray-600 mt-2">
                  We couldn't find any artworks matching your search or filter
                    ...
                    <div className="relative flex items-center justify-center bg-gray-100">
                      {art.image_url ? (
                        <img
                          ...
                          style={{
                            objectFit: "contain",
                            maxHeight: art.image_url.includes("portrait")
                              ? "500px"
                              : "300px",
                          }}
                        />
            ...
            <div className="flex flex-col md:flex-row justify-between items-center py-4 mt-4">
              <p className="text-gray-600 mb-2 md:mb-0">
                ...
                {Math.min(currentPage * itemsPerPage, filteredArts.length)} of{" "}
                {filteredArts.length} items
              </p>
